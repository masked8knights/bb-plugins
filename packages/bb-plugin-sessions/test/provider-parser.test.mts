...
describe("provider record normalization", () => {
  it("parses Pi JSONL and metadata records with a shared session id", () => {
    ...
    expect(jsonl.session.providerSessionId).toBe("same-session");
    expect(jsonl.items).toHaveLength(1);
    expect(jsonl.session.toolCalls).toBe(1);
    expect(metadata.session.providerSessionId).toBe("same-session");
    expect(metadata.session.title).toBe("Metadata title");
    expect(metadata.session.model).toBe("pi-model");
    expect(metadata.session.cwd).toBe("/workspace");
    expect(metadata.session.messageCount).toBe(4);
  });
  ...
  it("extracts provider-reported usage and cost from metadata records", () => {
    const jsonl = parseProviderJsonl(
      "pi",
      "primary",
      "/tmp/pi-session.jsonl",
    ...
    expect(jsonl.session.providerSessionId).toBe("merge-1");
    expect(metadata.session.inputTokens).toBe(1000);
    expect(metadata.session.cachedInputTokens).toBe(300);
    expect(metadata.session.cachedWriteTokens).toBe(50);
    expect(metadata.session.totalTokens).toBe(1250);
    expect(metadata.session.toolCalls).toBe(7);
    expect(metadata.session.costUsd).toBe(0.05);
    expect(metadata.session.costEstimated).toBe(false);
    expect(metadata.session.archived).toBe(true);
    expect(metadata.session.status).toBe("completed");
  });

  it("accepts provisional JSONL and completed metadata lifecycle records", () => {
    const jsonl = parseProviderJsonl(
      "pi",
      "primary",
      "/tmp/pi-session.jsonl",
    ...
    expect(metadata.session.status).toBe("completed");
    expect(metadata.session.providerSessionId).toBe("state-1");
  });
