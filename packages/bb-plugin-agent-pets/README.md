...
One shared 3D companion for BB agent sessions. The user creates the pet during
first-run onboarding by asking an agent for a named dog, cat, or capybara.

After creation, agents can read the pet's state, feed it, and talk to it. The
pet records agent and user interactions and responds with short,
species-appropriate phrases.
