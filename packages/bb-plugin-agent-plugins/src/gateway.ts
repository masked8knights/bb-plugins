...
export class McpGateway implements McpRuntime {
  ...
  private async ensureServer(pluginId: string, serverId: string): Promise<Connected | null> {
    ...
    if (!rec) return null;
    if (rec.enabled !== 1) throw new Error(`MCP server ${serverId} is disabled`);
    ...
    if (!validation.valid) throw new Error(`invalid server config for ${serverId}: ${validation.errors.join("; ")}`);
    // sse is unsupported in v0
    if (cfg.type === "sse") throw new Error(`sse transport not supported in v0 (server ${serverId})`);

    const p = this.connectServer(pluginId, serverId, cfg);
  ...
  private async connectServer(pluginId: string, serverId: string, cfg: ServerConfig): Promise<Connected> {
    ...
    try {
      ...
      if (cfg.type === "streamable-http") {
        ...
        // Resolve plugin-relative commands against PLUGIN_ROOT, independent of
        // the configured cwd. Stdio transports otherwise resolve ./ commands
        // relative to cwd, which violates the Agent Plugins contract.
        ...
        if (command.startsWith("./")) {
          if (!isWithinRoot(executable, pluginRoot)) throw new Error(`command escapes plugin root: ${command} -> ${executable}`);
        }
      ...
      const rec = this.store.listMcpServers(pluginId).find((s) => s.serverId === serverId);
      if (rec?.enabled === 1) {
        this.store.upsertMcpServer({ ...rec, status: "idle", lastError: null });
      }
      ...
      const rec = this.store.listMcpServers(pluginId).find((s) => s.serverId === serverId);
      if (rec?.enabled === 1) {
        this.store.upsertMcpServer({ ...rec, status: "error", lastError: msg });
      }
