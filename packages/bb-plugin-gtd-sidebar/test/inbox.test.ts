...
describe("filtering", () => {
  ...
  it("groups rows by first project appearance without reordering rows", () => {
    const groups = groupThreadsByProject([
      thread({ id: "a", projectId: "p2" }),
      thread({ id: "b", projectId: "p1" }),
      thread({ id: "c", projectId: "p2" }),
    ]);
    assert.deepEqual(
      groups.map((group) => [group.projectId, group.threads.map((candidate) => candidate.id)]),
      [
        ["p2", ["a", "c"]],
        ["p1", ["b"]],
      ],
