...
/** Group rows by first project appearance without changing row order. */
export function groupThreadsByProject<T extends { readonly projectId: string }>(
  threads: readonly T[],
  ...
  return [...groups].map(([projectId, groupedThreads]) => ({
    projectId,
