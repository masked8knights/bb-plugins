...
![GTD Sidebar running in BB](assets/staged-preview.png)

This screenshot is captured from the running BB application with the sidebar
provider selected and a seeded project/thread context. It shows the live GTD
inbox surface with **Group by project** enabled: project headings own the rows,
and the project metadata is removed from each card.
...
**From this fork** — install the plugin as a local path source. This is also
how you install a change that is not released yet:

```sh
cd /Users/patrick/workingdir/bb-plugins
pnpm install
pnpm --filter '@smsunarto/bb-plugin-gtd-sidebar' build
bb plugin install ./packages/bb-plugin-gtd-sidebar
```
...
Two lines: the title in bold when unread and a status slot, then the project (in
the flat view), the branch, activity counts, PR number, and the agent (which you
can turn off — see [Configuration](#configuration)). The status slot shows what the
...
### Scope and project grouping

The **All projects** scope picker filters the inbox to one project when needed.
The adjacent view button switches between **Show all projects** (the compact flat
list) and **Group by project**. In grouped mode, each Pinned, Next Action, Waiting,
Snoozed, and Settled shelf keeps its own project headings; cards no longer repeat
the project name.
...
- A project scope picker and an adjacent view-mode menu for flat or project-grouped rows.
...
```sh
pnpm --filter '@smsunarto/bb-plugin-gtd-sidebar' typecheck
pnpm --filter '@smsunarto/bb-plugin-gtd-sidebar' test
```
