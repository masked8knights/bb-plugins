 ...
 * ships neither. It filters by the `searchQuery` prop and adds the project
 * scope picker plus the view-mode menu the host has no equivalent for.
 */
export function ThreadInbox({ activeThreadId, onNavigate, searchQuery }: PluginThreadListProps) {
  ...
  const groupedByProject = viewMode === "project";
  const viewModeLabel = groupedByProject ? "Group by project" : "Show all projects";

  const renderThreadCard = (thread: PluginSidebarThread) => (
    <ThreadCard
      key={thread.id}
      thread={thread}
      provider={providerInfoById.get(thread.providerId)}
      showProviderIcon={showProviderIcon}
      showProject={!groupedByProject}
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      {/* These are the controls the host has no equivalent for. Everything else
          in the chrome above — New thread, search — is bb's and stays bb's. */}
      <div className="flex shrink-0 items-center gap-1 px-2 pb-0.5">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              type="button"
              aria-label={`Sidebar view: ${viewModeLabel}`}
              title={viewModeLabel}
              className="flex size-6 shrink-0 items-center justify-center rounded-md text-muted-foreground/70 transition-colors hover:bg-sidebar-accent hover:text-sidebar-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-foreground"
            >
              <Icon name="FolderTree" className="size-3.5" aria-hidden="true" />
            </button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" aria-label="Sidebar view">
            <DropdownMenuRadioGroup
              value={viewMode}
              onValueChange={(value) => {
                if (value === "all" || value === "project") setViewMode(value);
              }}
            >
              <DropdownMenuRadioItem value="all">Show all projects</DropdownMenuRadioItem>
...
function ParkedShelf({
  ...
  const renderSlimRow = (thread: PluginSidebarThread) => (
    <SlimRow
      key={thread.id}
      thread={thread}
      isActive={thread.id === activeThreadId}
      ...
      {expanded ? (
        groupByProject ? (
          <div className="flex flex-col gap-1">
            {groupThreadsByProject(threads).map((group) => (
              <ProjectGroup
                key={group.projectId}
                projectId={group.projectId}
                projectName={projectNameById.get(group.projectId) ?? "No project"}
              >
                <ul className="flex flex-col gap-0.5">{group.threads.map(renderSlimRow)}</ul>
              </ProjectGroup>
            ))}
          </div>
        ) : (
          <ul className="flex flex-col gap-0.5">{threads.map(renderSlimRow)}</ul>
        )
      ) : null}
    </section>
  );
        ...
        <div className="flex flex-col gap-1">
          {groupThreadsByProject(threads).map((group) => (
            <ProjectGroup
              ...
              projectName={projectNameById.get(group.projectId) ?? "No project"}
            >
              <ul className="flex flex-col gap-1">{group.threads.map(renderThread)}</ul>
            </ProjectGroup>
          ))}
        </div>
      ) : (
        <ul className="flex flex-col gap-1">{threads.map(renderThread)}</ul>
      )}
    </section>
  );
}
...
function ProjectGroup({
  ...
  return (
    <div data-project-group={projectId}>
      <h3 className="px-2.5 pb-0.5 pt-1 text-2xs font-medium text-muted-foreground/80">
        {projectName}
      </h3>
      {children}
    </div>
